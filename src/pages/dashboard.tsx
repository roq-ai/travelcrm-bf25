...
import { HotelListPage as ShowCaseList } from 'pages/hotels';
...
function DashboardPage() {
  ...
  const itineraryStat = useSWR<PaginatedInterface<ItineraryInterface>>(
    () => `/itineraries`,
    () => getItineraries({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Itineraries',
      result: itineraryStat?.data?.totalCount,
      isLoading: itineraryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`hotels`} passHref legacyBehavior>
                  <Button
